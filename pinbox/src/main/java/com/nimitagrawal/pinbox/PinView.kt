...
class PinView @JvmOverloads constructor(
    ...
    private val circleItemRadius: Int
    ...
    init {
        ...
        circleItemRadius = a.getDimension(R.styleable.PinView_circleItemRadius, 40f).toInt()
    ...
    override fun onTextChanged(
        ...
        mTransformed =
            transformation?.getTransformation(getText(), this)?.toString() ?: getText().toString()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        canvas.save()
        updatePaints()
        Companion.drawPinView(this, canvas)
        canvas.restore()
    }
    ...
    private fun drawPinCircle(canvas: Canvas, i: Int) {
        /* if(j == 1){
...
*/
        ...
        canvas.drawCircle(cx, cy, circleItemRadius.toFloat(), mPaint)
        updatePinBoxPath(i)
    }
    ...
    private fun drawTextAtBox(canvas: Canvas, paint: Paint?, text: CharSequence?, charAt: Int) {
        ...
        val cx = mItemCenterPoint.x
        val cy = mItemCenterPoint.y
        val x = cx - Math.abs(mTextRect.width().toFloat()) / 2 - mTextRect.left
        val y = cy + Math.abs(
    ...
    companion object {
        ...
        private const val VIEW_TYPE_NONE = 3
        private const val VIEW_TYPE_CIRCLE = 2
        ...
        private fun drawPinView(pinView: PinView, canvas: Canvas) {
            val highlightIdx = pinView.text!!.length
            (0 until pinView.mPinItemCount).forEach { i ->
                val highlight = pinView.isFocused && highlightIdx == i
                pinView.mPaint.color =
                    if (!highlight) pinView.currentLineColor else pinView.getLineColorForState(*HIGHLIGHT_STATES)
                pinView.updateItemRectF(i)
                pinView.updateCenterPoint()
                canvas.save()
                if (pinView.mViewType == VIEW_TYPE_RECTANGLE) {
                    pinView.updatePinBoxPath(i)
                    canvas.clipPath(pinView.mPath)
                }
                pinView.drawItemBackground(canvas, highlight)
                canvas.restore()
                if (highlight) {
                    pinView.drawCursor(canvas)
                }
                if (pinView.mViewType == VIEW_TYPE_RECTANGLE) {
                    pinView.drawPinBox(canvas, i)
                } else if (pinView.mViewType == VIEW_TYPE_LINE) {
                    pinView.drawPinLine(canvas, i)
                } else if (pinView.mViewType == VIEW_TYPE_CIRCLE) {
                    pinView.drawPinCircle(canvas, i)
                }
                if (DBG) {
                    pinView.drawAnchorLine(canvas)
                }
                if (pinView.mTransformed!!.length > i) {
                    if (pinView.transformationMethod == null && pinView.isPasswordHidden) {
                        pinView.drawCircle(canvas, i)
                    } else {
                        pinView.drawText(canvas, i)
                    }
                } else if (!TextUtils.isEmpty(pinView.hint) && pinView.hint.length == pinView.mPinItemCount) {
                    pinView.drawHint(canvas, i)
                }
            }

            // highlight the next item
            if (pinView.isFocused && pinView.text!!.length != pinView.mPinItemCount && pinView.mViewType == VIEW_TYPE_RECTANGLE) {
                val index = pinView.text!!.length
                pinView.updateItemRectF(index)
                pinView.updateCenterPoint()
                pinView.updatePinBoxPath(index)
                pinView.mPaint.color = pinView.getLineColorForState(*HIGHLIGHT_STATES)
                pinView.drawPinBox(canvas, index)
            }
        }
    }
}